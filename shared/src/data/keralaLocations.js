...
// ── Lookup Helpers ───────────────────────────────────────────────────────────
export function findDistrict(districtId) {
    return KERALA_DISTRICTS.find((d) => d.id === districtId);
}
export function findTaluk(talukId) {
...
export function findLocalBody(localBodyId) {
    for (const d of KERALA_DISTRICTS) {
...
export const getTaluks = (districtId) => KERALA_DISTRICTS.find((d) => d.id === districtId)?.taluks.map(({ id, name }) => ({ id, name })) ?? [];
export const getLocalBodies = (talukId) => {
    for (const d of KERALA_DISTRICTS) {
        const t = d.taluks.find((t) => t.id === talukId);
        if (t)
            return t.localBodies.map(({ id, name, type }) => ({ id, name, type }));
    }
    return [];
};
...
export const getDistrictName = (id) => KERALA_DISTRICTS.find((d) => d.id === id)?.name ?? id;
export const getTalukName = (talukId) => getTaluks(talukId.split("-").slice(0, 1).join("-")).find((t) => t.id === talukId)?.name ?? talukId;
//# sourceMappingURL=keralaLocations.js.map