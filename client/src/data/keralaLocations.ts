// Kerala Administrative Hierarchy — client-side bundle
// Matches server/src/data/keralaLocations.ts exactly.
// Used for cascading dropdowns (instant, no network round-trip needed).

export interface Ward       { id: string; name: string; }
export interface LocalBody  { id: string; name: string; type: "panchayat" | "municipality" | "corporation"; wards: Ward[]; }
export interface Taluk      { id: string; name: string; localBodies: LocalBody[]; }
export interface District   { id: string; name: string; taluks: Taluk[]; }
...
export const KERALA_DISTRICTS: District[] = [
  {
    ...
    taluks: [
      ...
      { id: "TVM-NYK", name: "Neyyattinkara", localBodies: [
        ...
        lb(["TVM-NYK-03","Karode Grama Panchayat","panchayat",13]),
      ]},
      { id: "TVM-VRK", name: "Varkala", localBodies: [
        ...
        lb(["TVM-VRK-03","Vakkom Grama Panchayat","panchayat",16]),
      ]},
  ...
  },
  {
    ...
    taluks: [
      ...
      { id: "ALP-KTN", name: "Kuttanad", localBodies: [
        ...
        lb(["ALP-KTN-02","Chambakulam Grama Panchayat","panchayat",15]),
      ]},
    ],
  },
  {
  ...
  },
  {
    ...
    taluks: [
      ...
      { id: "IDK-PRM", name: "Peerumade", localBodies: [
        ...
        lb(["IDK-PRM-02","Peerumade Grama Panchayat","panchayat",14]),
      ]},
    ],
  },
  {
    ...
    taluks: [
      ...
      { id: "EKM-ALV", name: "Aluva", localBodies: [
        ...
        lb(["EKM-ALV-03","Kadungalloor Grama Panchayat","panchayat",14]),
      ]},
      { id: "EKM-MVP", name: "Muvattupuzha", localBodies: [
        lb(["EKM-MVP-01","Muvattupuzha Municipality","municipality",34]),
        lb(["EKM-MVP-02","Koothattukulam Municipality","municipality",31]),
      ]},
    ],
  },
  {
    ...
    taluks: [
      { id: "TCR-TCR", name: "Thrissur", localBodies: [
        ...
        lb(["TCR-TCR-03","Puthur Grama Panchayat","panchayat",13]),
      ]},
  ...
  },
  {
    ...
    taluks: [
      { id: "MLP-MJR", name: "Manjeri", localBodies: [
        ...
        lb(["MLP-MJR-03","Tanur Municipality","municipality",31]),
      ]},
  ...
  // ── Wayanad (most detailed — demonstration focus) ──────────────────────────
  {
    id: "WYD", name: "Wayanad",
    taluks: [
  ...
  },
  {
    ...
    taluks: [
      { id: "KNR-KNR", name: "Kannur", localBodies: [
        ...
        lb(["KNR-KNR-03","Pappinisseri Grama Panchayat","panchayat",16]),
      ]},
      ...
      { id: "KNR-IRT", name: "Iritty", localBodies: [
        ...
        lb(["KNR-IRT-02","Aralam Grama Panchayat","panchayat",13]),
      ]},
    ],
  },
  {
    ...
    taluks: [
      { id: "KSD-KSD", name: "Kasaragod", localBodies: [
        ...
        lb(["KSD-KSD-03","Pallikkara Grama Panchayat","panchayat",14]),
      ]},
...
export const getDistricts = () => KERALA_DISTRICTS.map(({ id, name }) => ({ id, name }));

export const getTaluks = (districtId: string) =>
  KERALA_DISTRICTS.find((d) => d.id === districtId)?.taluks.map(({ id, name }) => ({ id, name })) ?? [];

export const getLocalBodies = (talukId: string) => {
  for (const d of KERALA_DISTRICTS) {
    const t = d.taluks.find((t) => t.id === talukId);
    if (t) return t.localBodies.map(({ id, name, type }) => ({ id, name, type }));
  }
  return [];
};

export const getWards = (localBodyId: string) => {
  for (const d of KERALA_DISTRICTS) {
    for (const t of d.taluks) {
      const lb = t.localBodies.find((lb) => lb.id === localBodyId);
      if (lb) return lb.wards;
    }
  }
  return [];
};

export const getDistrictName  = (id: string) => KERALA_DISTRICTS.find((d) => d.id === id)?.name ?? id;
export const getTalukName     = (talukId: string) => getTaluks(talukId.split("-").slice(0, 1).join("-")).find((t) => t.id === talukId)?.name ?? talukId;
